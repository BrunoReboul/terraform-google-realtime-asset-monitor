...
resource "google_monitoring_custom_service" "ram" {
  ...
  service_id   = "ram"
  display_name = "Real-time Asset Monitor"
}
...
resource "google_monitoring_slo" "ram_e2e_latency" {
  ...
  slo_id              = "ram-${each.value.origin}-e2e-latency"
  display_name        = "${tostring(each.value.goal * 100)}% of changes over the last ${each.value.rolling_period_days} days should be analyzed in ${each.value.origin} mode in less than ${each.value.threshold_str}"
