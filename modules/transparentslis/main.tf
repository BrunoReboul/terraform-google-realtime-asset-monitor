...
resource "google_monitoring_slo" "availability" {
  ...
  display_name        = "${lower(replace(each.value.method, ".", "-"))} ${tostring(each.value.goal * 100)}% of responses over the last ${each.value.rolling_period_days} days should be served successfully"
